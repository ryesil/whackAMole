...
public class AlbumRY {
    private String name;
    private String artist;
    private List<SongRY> songList;

    public AlbumRY(String name, String artist) {
        this.name = name;
        this.artist = artist;
    ...
    public boolean addSong(String title, double duration) {
        int sizeBefore = this.songList.size();
        this.songList.add(new SongRY(title, duration));
        return this.songList.size() == (sizeBefore + 1) ;
    }

    public SongRY findSong(String title) {
        return this.songList.stream().filter(song -> song.getTitle().equals(title)).collect(Collectors.toList()).get(0);
    }
